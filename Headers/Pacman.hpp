...
class Pacman
{
	unsigned char lastDirection = 255;
...
public:
	...
	unsigned char getOppositeDirection(unsigned char dir) const;
	unsigned char getANYDirection(unsigned char dir) const;
	...
	bool update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, std::vector<Position> ghostPositions, bool allowTurnBack, std::vector<bool> ghostFriten);
